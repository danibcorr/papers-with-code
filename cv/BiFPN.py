...
class BiFPN(layers.Layer):
    ...
    def __init__(
        ...
        self.layernorms = [
            layers.LayerNormalization(name=f"{self.name_layer}_layernorm_{i}")
            for i in range(4)
        ]
        ...
        self.upsamplings = [
            layers.Conv2DTranspose(
                filters=self.W_bifpn,
                kernel_size=2,
                strides=2,
                padding="same",
                name=f"{self.name_layer}_upsampling_{i}",
            )
            for i in range(2)
        ]
        ...
        self.downsamplings = [
            APSDownsampling(
                name=f"{self.name_layer}_downsampling_{i}", filters=self.W_bifpn
            )
            for i in range(2)
        ]
        ...
        self.conv1x1s = [
            layers.Conv2D(
                ...
                name=f"{self.name_layer}_conv1x1_{i}",
            )
            for i in range(3)
        ]
        ...
        self.conv2_td = layers.SeparableConv2D(
            filters=self.W_bifpn,
            kernel_size=kernel_size_convs,
            name=f"{self.name_layer}_conv2_td",
        )
        self.conv1_out = layers.SeparableConv2D(
            filters=self.W_bifpn,
            kernel_size=kernel_size_convs,
            name=f"{self.name_layer}_conv1_out",
        )
        self.conv2_out = layers.SeparableConv2D(
            filters=self.W_bifpn,
            kernel_size=kernel_size_convs,
            name=f"{self.name_layer}_conv2_out",
        )
        self.conv3_out = layers.SeparableConv2D(
            filters=self.W_bifpn,
            kernel_size=kernel_size_convs,
            name=f"{self.name_layer}_conv3_out",
        )

        # Weights for feature merging (learnable parameters)
        self.weights = {
            "w11": self.add_weight(
                f"{self.name_layer}_W11",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w21": self.add_weight(
                f"{self.name_layer}_W21",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w12": self.add_weight(
                f"{self.name_layer}_W12",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w22": self.add_weight(
                f"{self.name_layer}_W22",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w13": self.add_weight(
                f"{self.name_layer}_W13",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w23": self.add_weight(
                f"{self.name_layer}_W23",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w33": self.add_weight(
                f"{self.name_layer}_W33",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w14": self.add_weight(
                f"{self.name_layer}_W14",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
            "w24": self.add_weight(
                f"{self.name_layer}_W24",
                shape=[1, 1, 1, 1],
                initializer=tf.initializers.he_normal(),
                trainable=True,
            ),
    ...
    def get_config(self) -> dict:
        ...
        return {
            ...
            "kernel_size_convs": self.kernel_size_convs,
        }
    ...
    def call(self, p1: tf.Tensor, p2: tf.Tensor, p3: tf.Tensor) -> tuple:
        ...
        P_2_td = self.activation(
            self.layernorms[0](
                self.conv2_td(
                    self.padding(
                        temp
                        / (self.weights["w11"] + self.weights["w21"] + self.EPSILON)
                    )
                )
            )
        )
        ...
        temp = self.weights["w12"] * p1 + self.weights["w22"] * self.upsamplings[1](
            P_2_td
        )
        P_1_out = self.activation(
            self.layernorms[1](
                self.conv1_out(
                    self.padding(
                        temp
                        / (self.weights["w12"] + self.weights["w22"] + self.EPSILON)
                    )
                )
            )
        )
        ...
        temp = (
            self.weights["w13"] * self.downsamplings[0](P_1_out)
            + self.weights["w23"] * P_2_td
            + self.weights["w33"] * p2
        )
        P_2_out = self.activation(
            self.layernorms[2](
                self.conv2_out(
                    self.padding(
                        temp
        ...
        temp = self.weights["w14"] * p3 + self.weights["w24"] * self.downsamplings[1](
            P_2_out
        )
        P_3_out = self.activation(
            self.layernorms[3](
                self.conv3_out(
                    self.padding(
                        temp
                        / (self.weights["w14"] + self.weights["w24"] + self.EPSILON)
                    )
                )
            )
        )
...
class WeightedSum(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.weights = self.add_weight(
            shape=(self.num_outputs,), initializer="ones", trainable=True
        )
    ...
    def call(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        if not isinstance(inputs, list):
            ...
            raise ValueError(
                "A WeightedSum layer should be called on a list of inputs."
            )

        # Compute weighted sum of inputs
        weighted_inputs = tf.stack(inputs) * tf.reshape(
            tf.nn.softmax(self.weights), [self.num_outputs, 1, 1]
        )
