...
@njit
def cutmix_one_sample(
    train_ds_one: tuple, train_ds_two: tuple, alpha: float, beta: float
) -> tuple:
    """
...
def cut_mix(dataset_one: tuple, dataset_two: tuple, alpha: float, beta: float) -> tuple:
    ...
    for i in range(ds_one.shape[0]):
        ...
        images[i], labels[i] = cutmix_one_sample(
            (ds_one[i], labels_one[i]), (ds_two[i], labels_two[i]), alpha, beta
        )
