...
class APSLayer(layers.Layer):
    ...
    def downsample(self, inputs: tf.Tensor) -> tuple:
        ...
        polyphase_components = tf.TensorArray(
            tf.float32, size=self.stride**2, dynamic_size=False, clear_after_read=False
        )
        input_shape = tf.shape(inputs)

        # Gather polyphase components
        for arr_index, (i, j) in enumerate(
...
class APSDownsampling(layers.Layer):
    ...
    def __init__(self, name: str, filters: int):
        ...
        self.downsampling = APSDownsampleGivenPolyIndices(
            name=name + "APSDownsampleGivenPolyIndices_Layer"
        )

        self.conv_y = layers.Conv2D(
            kernel_size=3, strides=1, filters=filters, name=name + "APS_conv_y"
        )
        ...
        self.conv_x = layers.Conv2D(
            kernel_size=3, strides=1, filters=filters, name=name + "APS_conv_x"
        )
