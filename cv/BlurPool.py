...
class MaxBlurPooling2D(layers.Layer):
   ...
   def build(self, input_shape):
       ...
       if self.kernel_size == 3:
           ...
           raise ValueError(f"Unsupported kernel_size {self.kernel_size}. Supported sizes: 3, 5.")
   ...
   def call(self, x: tf.Tensor) -> tf.Tensor:
       ...
       x = tf.nn.pool(x, window_shape=(self.pool_size, self.pool_size), strides=(1, 1), padding='SAME',
                      pooling_type='MAX', data_format='NHWC')
       x = tf.nn.depthwise_conv2d(x, self.blur_kernel, strides=(self.pool_size, self.pool_size), padding='SAME')
...
class AverageBlurPooling2D(layers.Layer):
   ...
   def call(self, x: tf.Tensor) -> tf.Tensor:
       ...
       x = tf.nn.pool(x, window_shape=(self.pool_size, self.pool_size), strides=(1, 1), padding='SAME',
                      pooling_type='AVG', data_format='NHWC')
       x = tf.nn.depthwise_conv2d(x, self.blur_kernel, strides=(self.pool_size, self.pool_size), padding='SAME')
...
class BlurPool2D(layers.Layer):
   ...
   def call(self, x: tf.Tensor) -> tf.Tensor:
       ...
       return tf.nn.depthwise_conv2d(x, self.blur_kernel, strides=(self.pool_size, self.pool_size), padding='SAME')
